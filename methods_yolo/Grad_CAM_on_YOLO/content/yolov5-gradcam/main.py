...
parser.add_argument('--target-layer', type=str, default='model.model.24.m.2',
...
def get_res_img(bbox, mask, res_img):
    mask = mask.squeeze(0).mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).detach().cpu().numpy().astype(
        np.uint8)
    # Check for NaN or Inf values in the mask
    if np.isnan(mask).any() or np.isinf(mask).any():
        print("[WARNING] Invalid values encountered in the mask. Skipping visualization.")
        return res_img, np.zeros_like(mask)
